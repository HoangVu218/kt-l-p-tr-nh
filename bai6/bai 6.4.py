...
class RomanToInteger(object):
    ...
    def convert(self, roman_numeral):
        ...
        while i < len(roman_numeral):
            s1 = self.roman_map[roman_numeral[i]] # Giá trị của ký tự hiện tại
            
            # Kiểm tra xem còn ký tự tiếp theo hay không
            if i + 1 < len(roman_numeral):
                s2 = self.roman_map[roman_numeral[i+1]] # Giá trị của ký tự tiếp theo
...
print("IX ->", converter.convert("IX"))     # Expected: 9
print("LVIII ->", converter.convert("LVIII")) # Expected: 58 (50 + 5 + 1 + 1 + 1)
print("MCMXCIV ->", converter.convert("MCMXCIV")) # Expected: 1994 (1000 + 900 + 90 + 4)
